...
date: 2022-07-30T23:38:51+09:00
author: [hum_op]
tags: [infra]
---
...
![](/images/hashicorp-vault-oidc-auth-backend/keycloak-login.png)

### client の追加

左メニューから Clients を選択し、テーブル右上の`Create`ボタンを押下します。
![](/images/hashicorp-vault-oidc-auth-backend/keycloak-clients.png)

`Client ID`に`vault-oidc`、`Client Protocol`に`openid-connect`を入力し`Save`ボタンを押下します。

![](/images/hashicorp-vault-oidc-auth-backend/keycloak-create-client.png)
...
![](/images/hashicorp-vault-oidc-auth-backend/keycloak-client-settings.png)
...
![](/images/hashicorp-vault-oidc-auth-backend/keycloak-admin-set-email.png)
...
![](/images/hashicorp-vault-oidc-auth-backend/keycloak-add-user.png)

作成されるとユーザーの設定ページに遷移します。
Credentials タブをクリックしパスワードをセットします。

![](/images/hashicorp-vault-oidc-auth-backend/keycloak-user-set-password.png)
...
![](/images/hashicorp-vault-oidc-auth-backend/vault-access-auth-methods.png)

OIDC を選択し`Next`ボタンを押下します。

![](/images/hashicorp-vault-oidc-auth-backend/vault-new-auth-method-oidc.png)

Path は`oidc`のまま`Enable Method`ボタンを押下します。

![](/images/hashicorp-vault-oidc-auth-backend/vault-enable-oidc-method.png)
...
![](/images/hashicorp-vault-oidc-auth-backend/keycloak-credentials.png)

![](/images/hashicorp-vault-oidc-auth-backend/vault-configure-oidc.png)
...
![](/images/hashicorp-vault-oidc-auth-backend/vault-login-oidc.png)

ログインウィンドウが開くので、作成したユーザーのユーザー名とパスワードを入力します。

![](/images/hashicorp-vault-oidc-auth-backend/keycloak-oidc-login-window.png)

ログインに成功すると secret engine の一覧などが見れるようになります。

![](/images/hashicorp-vault-oidc-auth-backend/vault-secret-engines-after-login.png)

## ロールのポリシーについて

ロールを作成する際ポリシーを空で作成しました。そのため、Root Token でログインしたときに見えていた kv の`secret/`が OIDC でログインしたユーザーでは見えません。

![](/images/hashicorp-vault-oidc-auth-backend/vault-root-secret-engines.png)
...
![](/images/hashicorp-vault-oidc-auth-backend/vault-oidc-policy.png)
